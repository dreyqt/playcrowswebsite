        ...
        {ANNOUNCEMENTS.map(ann => (
          <button key={ann.id} onClick={() => navigate({ view: 'announcement', id: ann.id })} className="text-left w-full bg-transparent border-none cursor-pointer group" style={{ padding: 0 }}>
            <div className="flex items-start justify-between gap-4 p-6 transition-all duration-200" style={{ border: '1px solid rgba(212,169,77,0.1)', background: '#15110A' }} onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.background = '#1A140D'; (e.currentTarget as HTMLDivElement).style.borderColor = 'rgba(212,169,77,0.22)' }} onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.background = '#15110A'; (e.currentTarget as HTMLDivElement).style.borderColor = 'rgba(212,169,77,0.1)' }}>
              <div>
                <p className="font-cinzel font-semibold text-white mb-2" style={{ fontSize: 15, letterSpacing: '0.05em', color: '#EAD9B8' }}>{ann.title[lang]}</p>
                <p className="font-inter font-light mb-2" style={{ fontSize: 14, color: '#7A6248' }}>{ann.preview[lang]}</p>
                <p className="font-cinzel" style={{ fontSize: 11, color: '#3A2C18', letterSpacing: '0.15em' }}>{ann.date[lang]}</p>
              </div>
