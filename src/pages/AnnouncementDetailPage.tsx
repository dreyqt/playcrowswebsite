...
export default function AnnouncementDetailPage({ id, navigate, goHome }: { id: string; navigate: (p: Page) => void; goHome: () => void }) {
  ...
  return (
    <PageShell>
      ...
      <div className="mb-2"><p className="font-cinzel" style={{ fontSize: 11, color: '#1E3A5A', letterSpacing: '0.15em' }}>{ann.date}</p></div>
      <h1 className="font-cinzel font-bold text-white mb-8" style={{ fontSize: 'clamp(1.4rem, 3vw, 2rem)', letterSpacing: '0.08em', lineHeight: 1.3 }}>{ann.title}</h1>
